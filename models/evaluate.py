...
def evaluate_pre(val_dataloader, model_clip, MyModel, device, args, val_obj_list_post, PFL_TextEncoder, tokenizer, stage):
    ...
    for items in val_dataloader:
        ...
        with torch.no_grad():
            ...
            temp_cls = 0
            model_out = MyModel(text_embeddings, image_features, patch_tokens, stage = stage, mode = "val")
            if isinstance(model_out, (tuple, list)):
                if len(model_out) == 2:
                    pro_img, anomaly_maps_list = model_out
                elif len(model_out) == 3:
                    pro_img, anomaly_maps_list, _ = model_out
                else:
                    raise ValueError(f"Unexpected MyModel output length: {len(model_out)}")
            else:
                raise TypeError(f"Unexpected MyModel output type: {type(model_out)}")
