    ...
    def __init__(self, vision_width, text_width, num_layers=4):
        ...
        if self.text_width != self.vision_width:
            self.global_proj = nn.Sequential(
                nn.Linear(self.text_width, self.vision_width),
        ...
        # 最终的投影层，将融合后的特征投影到text_width维度
        self.final_proj = nn.Sequential(
            nn.Linear(vision_width, text_width),
            nn.LayerNorm(text_width)
        )
        
    def forward(self, patch_tokens, global_feature=None):
        """
        Args:
            patch_tokens: list of patch features from different layers, each with shape [B, N, C]
            global_feature: global image feature [B, C], optional
        Returns:
            fused_feature: [B, text_width]
        """
        B = patch_tokens[0].shape[0]
        ...
        # 聚合所有层的特征
        aggregated = torch.stack(fused_features, dim=1)  # [B, num_layers, C]
        aggregated = torch.mean(aggregated, dim=1)  # [B, C]
        
        # 投影到text维度
        output = self.final_proj(aggregated)  # [B, text_width]
...
class Context_Prompting(nn.Module):
    def __init__(self, args):
        ...
        self.voga_blocks = nn.ModuleList([
            ...
            for _ in range(len(self.args.features_list))
        ])
    ...
    def _apply_voga(self, patch_tokens):
        ...
        for idx, tokens in enumerate(patch_tokens):
            ...
            if side * side != seq_len:
                out_tokens.append(tokens)
                continue
            x = tokens.view(bsz, side, side, channels).permute(0, 3, 1, 2).contiguous()
            x = self.voga_blocks[idx](x)
            x = x.permute(0, 2, 3, 1).contiguous().view(bsz, seq_len, channels)
            out_tokens.append(x)
        return out_tokens
    ...
    def forward(self, text_features, image_features, patch_tokens, stage, mode):
        patch_tokens = self._apply_voga(patch_tokens)
        if stage == 1:
