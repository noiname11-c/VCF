...
from loss import FocalLoss, DiceLoss, Orthogonal_Loss, SoftFbetaLoss, DynamicBalancedBCELoss, PairwiseLogisticLoss
...
def train(args):
    ...
    loss_bal_bce_function = DynamicBalancedBCELoss(max_pos_weight=20.0)
    loss_soft_fbeta_function = SoftFbetaLoss(beta=1.5)
    loss_rank_function = PairwiseLogisticLoss(tau=0.2)
    ...
    for epoch_post in range(epochs):
        ...
        for items in train_bar:
            ...
            # Dynamic balanced BCE + soft-Fbeta + pairwise ranking.
            # Rank loss uses warmup to avoid destabilizing early training.
            rank_warmup = min(1.0, float(epoch_post + 1) / 5.0)
            loss_class = (loss_bal_bce_function(pro_img, anomaly)
                          + 0.4 * loss_soft_fbeta_function(pro_img, anomaly)
                          + (0.2 * rank_warmup) * loss_rank_function(pro_img, anomaly))
        ...
        if (epoch_post + 1) > 15 and (epoch_post + 1) % args.save_freq == 0:
