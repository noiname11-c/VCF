...
datasets_name_list = ["YAFID","mvtec","visa","BSD","RSDD", "KSDD2", "ISIC", "ClinicDB", "ColonDB", "HeadCT", "TN3K", "DTD", "Endo", "BrainMRI", "Br35H", "Kvasir", "BTAD"]
...
def visualization(save_root, pic_name, raw_image, raw_anomaly_map, raw_gt, the = 0.5, size = 518):
    ...
    npy_name = os.path.splitext(pic_name)[0] + ".npy"
    np.save(os.path.join(save_npy, "vis_map_" + npy_name), map)
    np.save(os.path.join(save_npy, "gt_" + npy_name), gt)
...
def calcuate_metric_pixel(results, obj_list, logger, alpha = 0.9, sigm = 4, args = None):
    ...
    for obj in obj_list:
        ...
        pr_px = pr_px.squeeze()
        if gt_px.ndim == 2:
            gt_px_for_pro = gt_px[None, ...]
            pr_px_for_pro = pr_px[None, ...]
        else:
            gt_px_for_pro = gt_px
            pr_px_for_pro = pr_px
        aupro_px = cal_pro_score(gt_px_for_pro, pr_px_for_pro)
        ...
        for i in range(len(img_path_list)):
            ...
            visualization(save_root= save_vis, pic_name=filename, raw_image= vis_img, raw_anomaly_map= np.squeeze(pr_px_for_pro[i]), raw_gt= np.squeeze(gt_px_for_pro[i]), the = best_threshold)
